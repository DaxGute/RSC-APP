/**
 * Map tab: localized strings for timeline filters, panels, alerts, and accessibility labels.
 * Consumed by `SsfAirQualityScreen` and related map UI components.
 */
...
const AQI_CATEGORY_LABELS: Record<AppLanguage, readonly string[]> = {
  en: [
    'Good',
    'Moderate',
...
/** Map an English EPA category label to the active app language. */
...
/** All user-facing string keys for the SSF air-quality map screen. */
export type MapScreenContent = {
...
/** English and Spanish content bundles for the map screen. */
export const mapScreenContent: Record<AppLanguage, MapScreenContent> = {
  en: {
    timeFilterDay: 'Day',
...
export function displayDayFilterLabel(internal: string, content: MapScreenContent): string {
  if (internal === 'Today') return content.today;
  if (internal === 'Yesterday') return content.yesterday;
  const m = internal.match(/^(\d+) Days Ago$/i);
  if (m) return content.daysAgo(Number.parseInt(m[1]!, 10));
  return internal;
}

/** Display label for internal month filter keys (`This Month`, `Jan '24`, …). */
export function displayMonthFilterLabel(internal: string, language: AppLanguage, content: MapScreenContent): string {
  if (internal === 'This Month') return content.thisMonth;
...
/** Localized three-letter month abbreviation for timeline chart ticks. */
