...
/** One timeline step: kriging rows plus UI opacity / uncertainty hints. */
...
/** Inputs for a full Now … +5h projection run. */
...
/** Output of `generateAnalogProjectionFrames` including quality metadata. */
export type GenerateAnalogProjectionFramesResult = {
  frames: ProjectionFrame[];
  /** Anchor-hour sample count at +1h (climatology prior strength). */
  analogCount: number;
  windAvailable: boolean;
  quality: AnalogProjectionQuality;
};

/** Max blend weight for recent live trend by future step (+1h … +5h). */
const LIVE_WEIGHTS = [0.6, 0.45, 0.3, 0.2, 0.1] as const;
/** Max blend weight for hour-of-day historical prior by step. */
const HISTORY_WEIGHTS = [0.2, 0.3, 0.35, 0.35, 0.35] as const;
/** Minimum anchor-hour samples before applying east/central/west zone medians. */
const MANY_SAMPLE_ZONE_THRESHOLD = 18;
/** µg/m³ cap on combined live + history delta per step. */
const MAX_GLOBAL_DELTA_BY_STEP = [4, 6, 7, 8, 8] as const;
/** Fraction of wind-warped field mixed into each cell (conservative advection). */
const WIND_BLEND = 0.12;
/** Max grid-index displacement per wind warp pass. */
const WIND_MAX_CELLS = 0.6;

/** Thirds of the SSF lattice used for zone-specific climatology. */
type ZoneName = 'west' | 'central' | 'east';

/** Hour-of-day prior for one future horizon from historical analog deltas. */
...
/** Median deltas at the current clock hour, indexed by future step. */
...
/** Linear-interpolated quantile on a numeric array. */
...
/** Mean of a flat analog delta vector. */
...
/** Spatial mean of a flat delta within one lattice third. */
...
/** Aggregate historical deltas for samples sharing `anchorHour` (0–23). */
...
/** Smooth west/central/east weights along column index `xi`. */
...
/** Light wind advection: blend base field with a capped `warpFieldByWind` shift. */
...
/** Domain-center u/v at the forecast slice nearest `minutesAhead`. */
...
/** Shared inputs reused while building each projection step. */
...
/** Build one `ProjectionFrame` (Now passthrough or blended future grid). */
...
/** Fallback quality when the current grid cannot be parsed. */
