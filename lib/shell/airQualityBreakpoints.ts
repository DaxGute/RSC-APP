/**
 * Canonical air-quality breakpoint and color definitions (US EPA AQI from PM2.5).
 * Single source of truth for AQI index and PM2.5 concentration categories.
 * Ported from SSF-AQI `aqi_utils.py` / `aqi_panel.py`.
 */

/** Display tuple for one EPA AQI index band (label + background / foreground hex). */
...
/** Background hex per AQI category — parallel to `PM25_CATEGORY_COLORS`. */
...
/** EPA piecewise-linear AQI from PM2.5 (µg/m³); null when input is missing or out of range. */
...
/** Human-readable band label for education copy (e.g. `AQI 51-100` or `AQI 301+`). */
...
/** Label and colors for an AQI index value; unknown / invalid inputs map to gray. */
...
/** True when `aqi` is at or above the reminder band selected in notification settings. */
...
/** Display tuple for one PM2.5 concentration band (includes map ring tint). */
...
/** Lower concentration bounds per PM2.5 band (µg/m³), for gradients and contours. */
...
/** Background hex per PM2.5 category — parallel to `AQI_CATEGORY_COLORS`. */
...
/** Label, colors, and ring tint for a PM2.5 concentration (EPA 0.1 µg/m³ binning). */
