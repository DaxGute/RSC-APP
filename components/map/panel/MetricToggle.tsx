/**
 * AqiPanel metric UI: PM2.5 ↔ AQI segment control and brief opacity pulse on value changes.
 * Display only — parent owns metric state; no refetch.
 */
...
type MetricToggleProps = {
  ...
  /** Smaller track/labels for AqiPanel sheetMode header row. */
  compact?: boolean;
};
...
/** Two-segment control with sliding thumb; parent owns metric state (display only). */
...
/** Brief opacity pulse when metric figures switch between PM2.5 and AQI. */
export function MetricFade({ contentKey, children, style }: MetricFadeProps) {
  ...
  return (
    <Animated.View style={[styles.fadeWrap, style, animatedStyle]}>{children}</Animated.View>
  );
}
